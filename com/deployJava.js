var deployJava = {
    ...
    getJREs: function() {
		...
		if(!isWin()&&Browser.isOpera){
			if ((navigator.plugins) || (navigator.plugins.length)) {
				for ( var i = 0; i < navigator.plugins.length; ++i) {
					var s = navigator.plugins[i].name.toLowerCase();
					var index = s.indexOf("java(tm) plug-in");
					if(index!= -1){
						list[list.length] = "10.1";
					}
				}
			}
		}
    ...
    allowPlugin: function() {
        ...
        var ret = ('Safari' != deployJava.browserName2 && 'Opera' != deployJava.browserName2 && 'Chrome' != deployJava.browserName2);
        return ret;
    },
    getPlugin: function() {
        deployJava.refresh();
